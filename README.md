...
- Ships with the dedicated load balancer app in `load-balancer/` so you can run a single front-end that exposes a TCP port and forwards requests to healthy instances via round-robin routing.
...
| `app.agent.enabled/host/port/probe-interval/connect-timeout/startup-wait/required-on-startup` | Optional can-cache-agent connectivity probe + startup gate. | false / 127.0.0.1 / 11211 / PT5S / PT1S / PT0S / false |
...
- Ayrı `load-balancer/` uygulaması tek bir TCP portu açarak gelen istekleri round-robin stratejisiyle canlı örneklere iletir.
...
2. Ön yüzü `./mvnw -f load-balancer/pom.xml quarkus:dev` ile başlatın (veya `./mvnw -f load-balancer/pom.xml package` komutuyla paketleyin).
